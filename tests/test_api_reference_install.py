...
def test_public_rc24_does_not_mask_an_unpublished_exact_rc32(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    identity = load_release_identity(REPO_ROOT)
    calls: list[str] = []
    ...
    def run_requirement(
        ...
        *,
        ...
        raise subprocess.CalledProcessError(
            1,
            ["python", "-c", "installed durable-workflow 2.0.0rc24, expected 2.0.0rc32"],
        )
    ...
    assert identity.registry_version == "2.0.0rc32"
    assert calls == [identity.exact_requirement]


def test_stale_documented_range_is_a_hard_failure_after_exact_rc32_exists(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    identity = load_release_identity(REPO_ROOT)
    calls: list[str] = []
