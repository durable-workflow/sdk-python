...
```bash
pip install 'durable-workflow[prometheus]~=2.0.0rc0'
```
...
The install commands above select the newest supported SDK prerelease. This
reference currently qualifies SDK `{{ durable_workflow_sdk_version }}` with
